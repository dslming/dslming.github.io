...
Cat = function(){
  // THREE.Group来操作一组对象，包括旋转，缩放，移动等，里面的子对象都会受到影响。
  this.threeGroup = new THREE.Group();
  // 实例化一个材质
  var yellowMat = new THREE.MeshLambertMaterial ({
    color: 0xfdd276, // 颜色
    shading:THREE.FlatShading // 阴影
  });
  ...
  this.handHeight = 10; // 手高度
  this.bodyHeight = 80; // 身体高度
  this.armHeight = ((this.bodyHeight * 3/5) - this.handHeight)/2 ; // 胳膊高度
  this.faceHeight = 30; // 脸高度
  this.shouldersPosition = new THREE.Vector3(0,this.armHeight*2 + this.handHeight, 0); // 肩膀位置
  this.isAttacking = false; // 攻击
  this.isFootReplacing = false; // 脚取代
  this.isBlinking = false; // 眨眼
  this.footUsed = "left";
