...
import { getHistoryNews } from "@/api/api";
...
export default {
  ...
  methods: {
    ...
    async getList(type) {
      let that = this;
      // 传递 type 参数，设置为1 就表示第一次加载，不传 type 表示加载历史消息
      if (type) {
        ...
        let dateStr = this.GetDateStr(--this.count);
         const res = await getHistoryNews({data:dateStr});
  ...
  components: {
    ...
    dropload,
    headTop,
    sidebar: () => import("@/components/sidebar") // 异步加载这个组件，只在需要的时候加载
  }
};
</script>
