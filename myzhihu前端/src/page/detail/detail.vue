...
<script>
import { transNumFormat } from '@/config/transNumFormat.js'
import attachImageUrl from '@/config/attachImageUrl.js'
...
export default {
    ...
    async created(){
        var res = await getNewsDetail({id:this.$route.query.id});
        this.res = res;
    },
    activated() {
        const that = this;
